...
public interface StudyLogRepository extends JpaRepository<StudyLog, Long> {
    ...
    @Query("SELECT sl.studyDate, SUM(sl.minutesStudied) FROM StudyLog sl WHERE sl.user.id = :userId AND sl.studyDate BETWEEN :startDate AND :endDate GROUP BY sl.studyDate ORDER BY sl.studyDate")
    List<Object[]> getDailyStudyMinutes(@Param("userId") Long userId, @Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);
}
